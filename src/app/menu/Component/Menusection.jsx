        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/12 w-full flex justify-center lg:block">
            <div className="menuucardd relative flex justify-center lg:justify-start">
              <h4 className="text-4xl font-normal text-[#da3036] lg:text-[62px] lg:leading-[64.17px] lg:absolute lg:left-0 lg:top-[215px] lg:-rotate-90">
                Menucard
              </h4>
            </div>
          </div>
          ...
          <div className="lg:w-7/12 w-full">
            <div className="old bg-[url('/old.webp')] bg-cover bg-center p-4 sm:p-6 rounded-lg h-full lg:w-7/8" data-aos="flip-left" data-aos-duration="1500">
              <h4 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6 text-center">Menu</h4>
          ...
          <div className="lg:w-6/12 w-full space-y-6">
            {activeMenu === null ? (
              ...
              <div className="rounded-lg p-6" data-aos="fade-left" data-aos-duration="1500">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-2xl text-center">{menuData[activeMenu].title}</h3>
                  <button onClick={handleBackClick} className="text-sm text-[#da3036] underline">Back</button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-lg">
                  {/** Split into 2 columns */}
                  <ul className="space-y-2">
                    {menuData[activeMenu].items.slice(0, Math.ceil(menuData[activeMenu].items.length / 2)).map((item, index) => (
                      <li key={index}>🍴 {item}</li>
                    ))}
                  </ul>
                  <ul className="space-y-2">
                    {menuData[activeMenu].items.slice(Math.ceil(menuData[activeMenu].items.length / 2)).map((item, index) => (
                      <li key={index}>🍴 {item}</li>
                    ))}
                  </ul>
                </div>
